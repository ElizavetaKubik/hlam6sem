	...
	for (int i = 0; i < 10; i++) 
	{
		try
		{
			...
			printf("%d-%02d-%02d %02d:%02d:%02d.%03d",
				...
				st.wMilliseconds);
			std::cout << std::endl;
			std::cout << "Difference in milliseconds : " << ( long(sysTime.wYear - st.wYear)*31536000000 + 
															  long(sysTime.wMonth - st.wMonth)*2592000000 +
															  long(sysTime.wDay - st.wDay)*86400000 +
															  long(sysTime.wHour - st.wHour)*3600000 +
															  long(sysTime.wMinute - st.wMinute) * 60000 +
															  long(sysTime.wMilliseconds - st.wMilliseconds)) << std::endl;
