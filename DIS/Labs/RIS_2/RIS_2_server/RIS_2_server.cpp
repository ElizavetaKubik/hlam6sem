	...
	try
	{
		...
		while (true) 
		{
			...
			if (ibuf.status == ibuf.ENTER || ibuf.status == ibuf.WAIT)
			{
				...
				if (!map.empty())
				{
					s = &((*(map.find(ibuf.fileName))).second);
				}
				if (s != NULL)
				{
				...
				else
				{
					...
					map.insert(pair<char*, ClientInfo>(ibuf.fileName, ibuf));
			...
			else if (ibuf.status == ibuf.LEAVE)
			{
				...
				map.erase(map.find(ibuf.fileName));
				ibuf.status = ibuf.LEAVE;
			}
			else if (ibuf.status == ibuf.INIT)
			{
				cout << "Init Critical Section " << ibuf.clientID << endl;
			}
			else if (ibuf.status == ibuf.NOINIT)
			{
				cout << "Destroy Critial Section " << ibuf.clientID << endl;
			}
